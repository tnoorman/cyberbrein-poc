...
from cyberbrein.collection.collector import CollectionError, CollectionSummary, CollectorService
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        dest="interface",
        required=True,
        help="Dedicated Wi-Fi interface to use for passive collection.",
    )
    parser.add_argument(
        "--database-path",
        "--db",
    ...
    parser.add_argument(
        ...
        default=True,
        help="Automatically enter and restore monitor mode (default: enabled).",
    )
...
def _collect(
    ...
    gpsd_provider: CachedGpsFixProvider | None,
) -> CollectionSummary:
    interface_manager = InterfaceManager(args.interface)
    interface_manager.prepare(auto_monitor=args.auto_monitor)
    try:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        summary = _collect(args, gpsd_provider)
        ...
        category = (
            error.category
            if isinstance(error, (CollectionError, InterfaceError))
